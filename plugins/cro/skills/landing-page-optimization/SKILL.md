...
**Cognitive Load & Halo Effect Insights:**

Apply these as a diagnostic lens over the research output. Full rationale at `~/.Codex/knowledgebase/website-design/principles.md`. Broader skill mapping at `~/.Codex/knowledgebase/website-design/applying-to-skills.md`.

- **50ms halo — one feeling, not five.** Visitors form an opinion in a fraction of a second; the hero carries the halo for the rest of the page. Ask: **what is the single feeling the hero must create in 50ms?** (Calm. Confident. Sharp. Fast.) Every variant gets one feeling, not a buffet. If the current hero tries to be three things at once, the optimization collapses it to one. [KB: principles.md → Halo Effect]
- **Declare war on cognitive load — whitespace = confidence.** Brains conserve energy; cognitive fluency reads as **higher quality and more trustworthy**. Walk every above-fold section and ask: **what can I remove?** Cut copy, simplify nav, one primary action per section. Hermès and Bottega Veneta use extreme whitespace as a confidence signal — "we don't need to shout." [KB: principles.md → Cognitive Load]
- **Peak-end via micro-interactions.** People remember a page by its peaks and how it ended. Spec at least one small moment of delight per variant: button hover/focus that feels intentional, form fields that confirm correct input (not just errors), satisfying CTA press states. These aren't extras — they drive remembered quality. [KB: principles.md → Peak-End Rule]
- **Repel the wrong customer as deliberately as you attract the right one.** Conversion isn't only "yes" — it's faster "no" from bad-fit visitors. Variants should pre-qualify: price anchors, specificity ("for teams of 50+"), tone that filters. A page that resonates with everyone resonates with no one. [KB: principles.md → Star of the Show]
