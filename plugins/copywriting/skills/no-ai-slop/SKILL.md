...
All output is British English. This overrides any US convention in the examples below.

- **Spelling:** -ise/-isation not -ize/-ization (organise, prioritise, optimisation). -our not -or (colour, behaviour, favour). -re not -er (centre, metre, theatre). -ce for nouns / -se for verbs (licence/license, practice/practise, advice/advise). Double the l before suffixes: travelled, cancelled, modelling, labelled. Also: analyse, catalogue, dialogue, programme (except computer program), grey, storey, cheque, tyre, kerb, enquiry, whilst → prefer "while", among → not amongst.
- **Exceptions that stay -ize/-ise as written:** proper nouns, product names, quoted text, code identifiers, and URLs. Never "correct" a quote or a brand.
- **Dates:** 30 July 2026 or 30/07/2026. Never July 30, 2026 or 07/30/2026.
- **Punctuation:** single quotes for quotes within quotes; full stops outside closing quotation marks unless the quote is a complete sentence. No Oxford comma unless it removes genuine ambiguity. Titles: Mr, Mrs, Dr with no full stop.
- **Money and units:** £ before the figure, no space (£1,200). Metric by default. Percentages as "20%".
- **Vocabulary:** prefer British terms over American where both exist — mobile not cell, holiday not vacation, flat not apartment, autumn not fall, boot/bonnet, ring not call, CV not résumé, post not mail, maths not math.
- **Collective nouns:** British usage allows plural agreement ("the team are shipping it"). Follow the writer's existing choice; don't standardise it.

If the draft is deliberately US English (an American client, a US-facing page, a quoted source), keep it US and say so in the What changed section. Otherwise convert.

## Calibration by content type

Match the depth of edit to the job. Slop-cutting is constant; the rest scales.

| Content | Edit depth | Notes |
|---|---|---|
| Blog posts, essays, LinkedIn | Full | Every principle applies. Protect voice hardest here. |
| Landing pages, sales copy | Full, but keep the sell | Don't cut a benefit, proof point, or CTA for being "puffery" — cut only the *unsupported* claim. Repetition of the offer is deliberate, not synonym cycling. |
| Local business service and location pages | Medium | Keep the town, trade, and service names repeated verbatim — that's SEO, not slop. Don't strip the location out of a location page. Keep phone numbers, hours, and coverage areas exactly as written. |
| Email campaigns | Full | Subject lines are exempt from the em dash and fragment rules if the writer's style calls for it. |
| Case studies, testimonials | Light | Never edit inside a quote. Never smooth a client's own words. |
| Legal, compliance, guarantees | Minimal | Cut banned words only. Do not restructure, do not "untangle" — the tangle may be load-bearing. |

## Words to cut

Banned outright: delve, foster, leverage, utilize, utilise, facilitate, empower, streamline, robust, cutting-edge, paradigm shift, game changer, this is huge, this changes everything, tapestry, realm, beacon, multifaceted, meticulous, intricate, paramount, transformative, elevate, embark, supercharge, harness, ever-evolving.
...
2. Identify the core point and 3-5 voice signals to preserve, such as vocabulary, cadence, bluntness, humour, uncertainty, or digressions. Keep this note internal. If you cannot identify the core point, ask the user.
3. Identify the content type and set the edit depth from the calibration table.
4. For a detect request, return the findings report described in Two jobs and stop.
5. For an edit, make the minimum effective changes, then run a British English pass over the result, then check the edited draft against `eval.md` yourself.
...
Adapted from [petergyang/no-ai-slop](https://github.com/petergyang/no-ai-slop) (MIT). Local additions: British English house style, content-type calibration.
