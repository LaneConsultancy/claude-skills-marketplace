...
# This local ffmpeg build does not include zscale. Keep the pipeline compatible
# by forcing a standard SDR pixel format and writing BT.709 metadata on encode.
TONEMAP_CHAIN = "format=yuv420p,setparams=colorspace=bt709:color_primaries=bt709:color_trc=bt709"
...
def extract_segment(
    ...
    if transform == "rotate_clockwise_fit_blur":
        fg = (
            "format=yuv420p,setparams=colorspace=bt709:color_primaries=bt709:color_trc=bt709,"
            "transpose=1,scale=1920:1080:force_original_aspect_ratio=decrease"
        )
        bg = (
            "format=yuv420p,setparams=colorspace=bt709:color_primaries=bt709:color_trc=bt709,"
            "transpose=1,scale=1920:1080:force_original_aspect_ratio=increase,"
            "crop=1920:1080,gblur=sigma=28,eq=brightness=-0.08:saturation=0.85"
        )
        post = f",{grade_filter}" if grade_filter else ""
        filter_complex = (
            f"[0:v]{bg}[bg];"
            f"[0:v]{fg}[fg];"
            f"[bg][fg]overlay=(W-w)/2:(H-h)/2,format=yuv420p{post}[v]"
        )
        cmd += ["-filter_complex", filter_complex, "-map", "[v]", "-map", "0:a:0"]
    else:
        cmd += ["-vf", vf]
    cmd += [
        "-af", af,
...
def extract_all_segments(
    ...
    has_transforms = any(r.get("transform") for r in edl["ranges"])
    if draft:
        clips_name = "clips_draft_rotated" if has_transforms else "clips_draft"
    elif preview:
        clips_name = "clips_preview_rotated" if has_transforms else "clips_preview"
    else:
        clips_name = "clips_graded_rotated" if has_transforms else "clips_graded"
    clips_dir = edit_dir / clips_name
